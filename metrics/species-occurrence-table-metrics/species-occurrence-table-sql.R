...
if (length(args) > 0) {
  for (i in seq_along(args)) {
    if (args[i] == "--previous-year" && i < length(args)) {
...
output_dir <- "../../ui/public/data/species-occurrence-table"
...
# Need FOUR downloads:
# 1. All data up to two_years_ago - records FROM the country (countryCode)
# 2. All data up to previous_year - records FROM the country (countryCode)
# 3. All data up to two_years_ago - records PUBLISHED BY the country (publishingcountry)
# 4. All data up to previous_year - records PUBLISHED BY the country (publishingcountry)

# Up to two_years_ago data - FROM country (countryCode)
sql_2024_from <- paste0("
SELECT 
  countryCode,
  CASE 
...
# Up to previous_year data - FROM country (countryCode)
sql_2025_from <- paste0("
SELECT 
  countryCode,
  CASE 
    WHEN classKey IN ('359') THEN 'Mammals'
...
# Up to two_years_ago data - PUBLISHED BY country (publishingcountry)
sql_2024_published <- paste0("
SELECT 
...
# Up to previous_year data - PUBLISHED BY country (publishingcountry)
sql_2025_published <- paste0("
SELECT 
  publishingcountry AS countryCode,
  CASE 
    WHEN classKey IN ('359') THEN 'Mammals'
...
existing_2024_from_downloads <- list.files(pattern = paste0("^occurrence-", two_years_ago, "-from-[0-9]+-[0-9]+\\.zip$"), full.names = TRUE)
existing_2025_from_downloads <- list.files(pattern = paste0("^occurrence-", previous_year, "-from-[0-9]+-[0-9]+\\.zip$"), full.names = TRUE)
existing_2024_published_downloads <- list.files(pattern = paste0("^occurrence-", two_years_ago, "-published-[0-9]+-[0-9]+\\.zip$"), full.names = TRUE)
existing_2025_published_downloads <- list.files(pattern = paste0("^occurrence-", previous_year, "-published-[0-9]+-[0-9]+\\.zip$"), full.names = TRUE)
...
if (length(existing_2024_from_downloads) > 0) {
  ...
  download_key_2024_from <- sub("^occurrence-2024-from-", "", sub("\\.zip$", "", basename(download_file)))
  cat("Found existing 2024 FROM country download file:", basename(download_file), "\n")
  cat("Using download key:", download_key_2024_from, "\n")
  
  data_2024_from <- rgbif::occ_download_get(download_key_2024_from, overwrite = FALSE) |>
...
if (length(existing_2025_from_downloads) > 0) {
  ...
  download_key_2025_from <- sub("^occurrence-2025-from-", "", sub("\\.zip$", "", basename(download_file)))
  cat("Found existing 2025 FROM country download file:", basename(download_file), "\n")
  cat("Using download key:", download_key_2025_from, "\n")
  
  data_2025_from <- rgbif::occ_download_get(download_key_2025_from, overwrite = FALSE) |>
    rgbif::occ_download_import()
...
if (length(existing_2024_published_downloads) > 0) {
  ...
  download_key_2024_published <- sub("^occurrence-2024-published-", "", sub("\\.zip$", "", basename(download_file)))
  cat("Found existing 2024 PUBLISHED BY country download file:", basename(download_file), "\n")
  cat("Using download key:", download_key_2024_published, "\n")
  
  data_2024_published <- rgbif::occ_download_get(download_key_2024_published, overwrite = FALSE) |>
    rgbif::occ_download_import()
...
if (length(existing_2025_published_downloads) > 0) {
  ...
  download_key_2025_published <- sub("^occurrence-2025-published-", "", sub("\\.zip$", "", basename(download_file)))
  cat("Found existing 2025 PUBLISHED BY country download file:", basename(download_file), "\n")
  cat("Using download key:", download_key_2025_published, "\n")
  
  data_2025_published <- rgbif::occ_download_get(download_key_2025_published, overwrite = FALSE) |>
    rgbif::occ_download_import()
...
# Capture attribution metadata for all downloads
cat("\nCapturing download attributions...\n")
download_attribution_2024_from <- get_download_attribution(download_key_2024_from)
download_attribution_2025_from <- get_download_attribution(download_key_2025_from)
download_attribution_2024_published <- get_download_attribution(download_key_2024_published)
download_attribution_2025_published <- get_download_attribution(download_key_2025_published)
...
# Helper function to process and write JSON files for a dataset
process_and_write_dataset <- function(data_2024, data_2025, published_by_flag, dataset_label, attribution_2024 = NULL, attribution_2025 = NULL) {
  success_count <- 0
  fail_count <- 0
  ...
  for (country_code in countries_to_export) {
    ...
    # Create the data structure for JSON output
    ...
    # Add download attributions if available
    attributions <- list()
    if (!is.null(attribution_2024)) attributions <- c(attributions, list(attribution_2024))
    if (!is.null(attribution_2025)) attributions <- c(attributions, list(attribution_2025))
    if (length(attributions) > 0) {
      occurrence_table_data$downloadAttributions <- attributions
    }
    ...
    # Write JSON file
    file_suffix <- ifelse(published_by_flag, "published", "from")
    output_file <- file.path(output_dir, paste0(country_code, "-", file_suffix, ".json"))
    
    cat("Writing JSON file:", basename(output_file), "...\n")
    tryCatch({
      write_json(occurrence_table_data, output_file, pretty = TRUE, auto_unbox = TRUE)
      cat("✓ Successfully wrote JSON file\n")
      success_count <- success_count + 1
    }, error = function(e) {
      cat("✗ Failed to write JSON file:", e$message, "\n")
...
# Process and write FROM country data
cat("\n=== Processing FROM Country Data ===\n")
results_from <- process_and_write_dataset(data_2024_from, data_2025_from, FALSE, "FROM country", download_attribution_2024_from, download_attribution_2025_from)

# Process and write PUBLISHED BY country data
cat("\n=== Processing PUBLISHED BY Country Data ===\n")
results_published <- process_and_write_dataset(data_2024_published, data_2025_published, TRUE, "PUBLISHED BY country", download_attribution_2024_published, download_attribution_2025_published)

cat("\n=== Summary ===\n")
cat("FROM Country Data:\n")
cat("  Successfully written:", results_from$success, "countries\n")
cat("  Failed to write:", results_from$fail, "countries\n")
cat("PUBLISHED BY Country Data:\n")
cat("  Successfully written:", results_published$success, "countries\n")
cat("  Failed to write:", results_published$fail, "countries\n")
